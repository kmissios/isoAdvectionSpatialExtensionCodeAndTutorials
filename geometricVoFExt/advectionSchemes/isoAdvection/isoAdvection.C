...
void Foam::isoAdvection::timeIntegratedFlux()
{
    ...
    // List of cells that are not interface cells but are downwind to an interface cell
    DynamicList<label> cellsDownwindToInterface;
    DynamicList<bool> isInterfaceCell;    
    // For each cell j in cellsDownwindToInterface donorCells is a list of interface cells upwind to cell j.
    DynamicList<DynamicList<label>> donorCells;
    // Populating cellsDownwindToInterface and donorCells
    const cellList& cells = mesh_.cells();
    // Populating isInterfaceCell
    forAll(cells, cellI)
    {   
        ...
        isInterfaceCell.append(boolValue);
    }
    ...
    forAll(cells, cellI)
    {
        ...
        DynamicList<label> upwindCells;
        const cell& celliFaces = cellFaces[cellI];
        
        forAll(celliFaces, fi)
        {
            label facei = celliFaces[fi];
            if (mesh_.isInternalFace(facei))
            {       
                if (cellI == own[facei])
                {
                    if (phiIn[facei] < 0)
                    {
                        upwindCells.append(mesh_.faceNeighbour()[facei]);
                        // upwindCells.append(mesh_.faceOwner()[facei]);
                    }
                }
                else
                {
        ...
        // why do you want the upwind cells 
        // since you exclude the interface cells does this work ? check it 

        DynamicList<label> upwindInterfaceCells;
        forAll(upwindCells, j) 
        {
            const label uCellI = upwindCells[j];
                
            if(isInterfaceCell[uCellI])
            { 
                upwindInterfaceCells.append(uCellI);
                continue;
            }            
        }

        if(upwindInterfaceCells.size() > 0)
        {
            cellsDownwindToInterface.append(cellI);
            donorCells.append(upwindInterfaceCells);
        }
    }
    

    forAll(cellsDownwindToInterface, i)
    {

        label cellI = cellsDownwindToInterface[i];
        DynamicList<label> donors = donorCells[i];
        
        //At the moment, we apply a very basic implementation where only 1 donor is required
        /*
        forAll(donors, j)
        {
            const label cellJ = donors[j];
            // Info << "Cell " << cellJ  << " is a donor cell to cell " << cellI << endl; 
            const point x0 = surf_->centre()[cellJ];
            vector n0 = -surf_->normal()[cellJ];
            n0 /= (mag(n0));
            const scalar Un0 = UInterp.interpolate(x0, cellJ) & n0;
            // Info << "Cell " << cellJ  << "'s isoface has centre " << x0 << ", normal " << n0 
            //	<< " and speed " << Un0 << endl;
        }*/
        // Now we can use this data to set (x0, n0, Un0) in cellI.
        // A very simple take (probably not final solution) would be to simply let cellI
        // inherrit (x0, n0, Un0) from donor[0]:
        const label cellJ = donors[0];
        if (mag(surf_->normal()[cellJ]) != 0)
        {
            const point x0 = surf_->centre()[cellJ];
            vector n0 = -surf_->normal()[cellJ];
            n0 /= (mag(n0));
            const scalar Un0 = UInterp.interpolate(x0, cellJ) & n0;
            // With (x0, n0, Un0) set in cellI we can now apply isoAdvection to its downwind faces
            // just as we did with the interface cells:
            const cell& celliFaces = cellFaces[cellI];
            forAll(celliFaces, fi)
            {
